...
# Files for writing
# Plain alignment 5'|-------|3'
f = open('str_alignment.txt', 'w')
# Fasta formatted
fasta = open('str_alignment.fa', 'w')
# Load gene
usg = list(SeqIO.parse(dir+'usg.fa', 'fasta'))[0]
gId = str(usg.id)
gseq = str(usg.seq)
gSeqLen = len(gseq)
...
positions = []
for ts in SeqIO.parse(dir+'ust.fa', 'fasta'):
	# Transcripts
	tseq = str(ts.seq)
	tId = str(ts.id)[0:15]
	pos = gseq.find(tseq)
	...
	# 5'-UTR
	for utr in SeqIO.parse(dir+'5utr.fa', 'fasta'):
		if str(utr.id) == tId:
			u5Seq = str(utr.seq)
			pos = gseq.find(u5Seq)
			if pos == -1: utrs = '\n'
	...
	# Exons
	for ex in SeqIO.parse(dir+'exons.fa', 'fasta'):
	...
	# 3'-UTR
	for utr in SeqIO.parse(dir+'3utr.fa', 'fasta'):
		if str(utr.id) == tId:
			u3Seq = str(utr.seq)
			pos = gseq.find(u3Seq)
			if pos == -1: utrs = '\n'
